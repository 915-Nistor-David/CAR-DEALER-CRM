...
**Status: Etapa 5 (permissions & accountability) done 2026-07-21**, on top of Etapa 4 (notifications, agenda, UI fixes, same day) and Etapa 3 (security + correctness hardening, same day). Etapa 2 (2026-07-18) built RBAC (Owner/Vanzari/Junior), granular pipeline stages, in-app notifications, reminders (RAR/documents/stuck-in-stage/stock-aging), vehicle documents checklist, employee activity report — all from the first dealer interview (log below). **Deferred until the seller-team meeting:** interested-clients (leads) database + auto-matching + wa.me message links; sales statistics page. Pricing question still open.
...
- Stage moves: **Owner+Vanzari only**, never on a sold vehicle, and **never into the `IsSoldStage`** — that stage is reachable only by recording a sale. Both guards live in `VehiclesController.ChangeStage`; `SalesController` moves the car through `IVehicleStageService` directly and is unaffected. Without the sold-stage guard you could park a car in "Vândută" with no `Sale` row: no price, no buyer, no profit, absent from `/sales`, and not even badged sold (`IsSold` comes from the sale, not the stage). Juniors moving cars was the original dealer request, but in practice a junior could send one straight to "Vândută" — David reversed the decision 2026-07-21. Frontend mirrors it: `Board` passes `canMove` into `useDraggable({ disabled })`, `VehicleDetail` hides the `StageMover` card. Vehicle create/edit + sales: Owner+Vanzari. Vehicle delete, users, stages, settings, reports: Owner only. `GET /api/sales` is 403 for Junior — Dashboard skips the sales fetch and hides profit tiles for non-Owner.
- **Costs carry their author** (`VehicleCost.CreatedByUserId`, nullable — rows predating the column show "—"). `CostDto` exposes `CreatedByName` and a server-decided `CanDelete`; **deletion is author-or-Owner**, so legacy authorless costs are Owner-only. Before this, any role could silently delete any cost and change a car's profit.
- **`VehicleDto.LastMovedBy` / `LastMovedAt`** answer the dealer's "who moves the cars" question without opening the history. Derived from `VehicleStatusHistory`, no new column: `GetAll` uses one greatest-n-per-group query (NOT EXISTS on a newer row) keyed by vehicle; `GetById` just reads the head of the already-sorted history list — which is why that list is ordered by `Timestamp` **and** `HistoryId`.
...
- `dotnet ef migrations add` fails while the API is running (the .exe is locked). If the running instance is *the user's*, don't kill it — `--configuration Release` builds to `bin/Release` and sidesteps the lock entirely. To then run a second API alongside it, set **both** `ASPNETCORE_URLS` and `ASPNETCORE_ENVIRONMENT=Development`: `--no-launch-profile` skips `launchSettings.json`, so the environment defaults to Production, user-secrets don't load, and `StartupConfigGuard` correctly refuses to boot.
...
## Etapa 5 — permissions & accountability (2026-07-21)
David hit two of these himself while using the app as a Junior. Built: stage moves restricted to Owner+Vanzari and the sold stage closed to manual moves entirely (see RBAC rules); cost author recorded and shown, with delete limited to author-or-Owner; `LastMovedBy`/`LastMovedAt` surfaced on the vehicle card and detail header.

Verified via API on a second instance (port 5101, Release): Junior → 403 on any move, including into a normal work stage; Owner and Vanzari → 400 with the "record the sale instead" message when targeting the sold stage, 200 on a normal stage; `POST /vehicles/{id}/sale` still moves the car into "Vândută" and computes profit, so the guard doesn't touch the sale path; a Junior's cost shows `createdByName: "Jonut Junior"` with `canDelete: true` for the Junior and for the Owner, `false` for the Junior on the Owner's cost — and the DELETE endpoint agrees (403 / 200 / 200); pre-migration costs come back `createdByName: null`, `canDelete: false` for non-Owner. Verified in the DOM: Junior's board shows the read-only subtitle and cards with `cursor: pointer` + `aria-disabled="true"`, no "Mută în altă etapă" card on the detail page, "Adăugat de" column populated, delete ✕ present only on own costs; as Owner the move card is back and picking "Vândută" surfaces the backend message inline while the car stays in Mecanică. No console errors.

**Mobile is not done** and was deliberately deferred (David's call, after the seller meeting). The app is desktop-first: 19 responsive breakpoints total, and the navbar is 8 links in a fixed flex row with no hamburger — it overflows off-screen at 375px, making Agendă/Etape unreachable. `VehicleForm`'s `grid-cols-2` and the Agenda calendar grid also don't collapse. Tables in Sales/Users/Activity degrade acceptably (`overflow-x-auto`). Test with F12 → Ctrl+Shift+M → iPhone SE; for a real phone, `npm run dev -- --host` plus a matching `VITE_API_URL` and CORS origin.
